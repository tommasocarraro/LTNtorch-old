...
def prepare_dataset():
    ...
    return ratings.to_numpy(), ratings_test.to_numpy(), seen_matrix, rate_matrix, sim_users_ratings, \
           sim_users_features, sim_items, items_info, users_info, item_popularity
...
class Likes(torch.nn.Module):
    ...
    in [0., 1.]. The higher the value the better the compatibility between the user and the item. The predicate takes
    as input the indexes of a user and an item and returns the scalar product of the two embedding. The result is then
    put in [0., 1.] by applying a sigmoid function on it. The result can be interpreted a a degree of compatibility
    between the user and the item fed in input.
    """
...
def main():
    ...
    likes = ltn.Predicate(Likes(users.shape[0], items.shape[0], 100)).to(ltn.device)
    # likes = ltn.Predicate(Likes(users.shape[0], items.shape[0], emb_size=100)).to(ltn.device)
